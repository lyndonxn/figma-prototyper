...
| T-06a | M6a：设计 IR 通道（toIR 注入 + RESULT.data + --ir-out 落盘） | BACKLOG | — | — | FUN-ACC-601~602 | 2026-09-22 新增，阻塞于用户启动指令；scope 见 spec/03 IR 契约 |
| T-06b | M6b：Design→Code 闭环（Agent 合成 HTML+CSS + Chrome headless 对比） | BACKLOG | — | — | FUN-ACC-603~604 | 阻塞于 T-06a；shot 子命令不依赖 Figma 可先契约测试 |
