...
| T-07a | M7a：图层重建（figmapt rebuild——IR→沙箱脚本确定性生成器 + skill Code→Design 工作流节） | DONE | M7a-impl agent-51b20e73 / 验收 agent-eea43a01、agent-01e66c01 | 2026-09-22 | FUN-ACC-701~702 | 701 静态+契约 pass（确定性 golden/映射覆盖/占位符污染回归，cli 32 全绿）；702 契约+运行时 pass（images 通道字节一致；U11 真机重建 44 节点、字体零回退、与原稿布局/文本/间距/对齐/底色等价，证据 screenshots/m7a-rebuild-u11.png）。运行时迭代修复三缺陷（复审 ACCEPT agent-01e66c01）：①IR 缺对齐语义 → toIR/rebuild 双侧补 layout.primary/counter + absolute（IR schema M7 扩展）②resize 必须在 layoutMode 后（否则 auto-layout sizing 被 hug 重置）③IR 无 fills 的帧须清 createFrame 默认白填充。已知边界：矢量图形包围盒近似（坑 26）。测试 59→67（bridge 35 + cli 32） |
| T-07b | M7b：DOM 抽取（figmapt extract——系统 Chrome + CDP→IR，cli 引入 ws） | DONE | M7b-impl agent-1ce8315e / 验收 agent-3a1268ad | 2026-09-23 | FUN-ACC-703~704 | 703 静态+契约 pass（映射纯函数单测 + --cdp-url 假 DevTools 桩测 + Chrome 缺失/参数错误路径，独立验收 ACCEPT agent-3a1268ad 并补 3 项验收测试）；704 运行时 pass（controller 代跑：m6-u11 HTML 真机 extract→rebuild（36:1051，37 created/0 skipped/字体零回退）→toIR 读回 diff——38/38 节点配对、0 mismatch、0 豁免、max bounds delta 0，证据 screenshots/m7b-roundtrip-u11.png + output/code/m7b-u11/）。运行时抓出并修复 getFlattenedDocument 不稳定缺陷（弃用，改 getDocument 嵌套树客户端拉平 + 盒模型重试；skill 坑 31、spec/03 已回填）。测试 67→75（bridge 35 + cli 40，真 Chrome 冒烟 gated skip 2） |
