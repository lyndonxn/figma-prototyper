...
| M7a | 图层重建：`figmapt rebuild`——IR→沙箱脚本确定性生成器（映射表见 `03`）+ skill Code→Design 工作流节 | M6a | FUN-ACC-701~702 | BACKLOG |
| M7b | DOM 抽取：`figmapt extract`——系统 Chrome + CDP（DOM/computed style/盒模型→IR），cli 侧引入 `ws` | M7a | FUN-ACC-703~704 | BACKLOG |
